...
import { enquiryMessage, whatsappUrl } from '../../lib/enquiry';

const directWhatsApp = whatsappUrl(brand.whatsapp, 'Hello Bhavana, I would like to enquire about Mystic Tattva guidance.');
...
export const Contact = ({ selection, onDialog }) => {
  ...
  useEffect(() => { if (selection) { setForm(prev => ({ ...prev, service: selection.service || '', message: selection.message || '' })); setError(''); } }, [selection]);
  ...
  const submit = (e) => {
    ...
    if (!options.includes(form.service)) { setError('Please select the guidance you are interested in.'); return; }
    const text = enquiryMessage(form);
    const url = whatsappUrl(brand.whatsapp, text);
    if (!url) { onDialog({ type: 'enquiry', text }); return; }
  ...
  return <section id="contact" className="contact section-pad" aria-labelledby="contact-title"><div className="page-width contact-grid">
    <Reveal className="contact-copy"><Chapter number="07" light>YOUR NEXT CHAPTER</Chapter><h2 id="contact-title" className="display-title" data-testid="contact-title">It begins with<br/><em>a conversation.</em></h2><p data-testid="contact-description">You don’t have to have it all figured out.<br/>Share what’s on your mind, and take the first step towards a little more clarity.</p><div className="contact-note" data-testid="contact-process"><Star size={23}/><p>A personal conversation.<br/>A thoughtful approach.<br/><span>Guidance, at your pace.</span></p></div><Button className="whatsapp-button" data-testid="whatsapp-contact" onClick={() => directWhatsApp ? window.location.assign(directWhatsApp) : onDialog({ type: 'whatsapp' })}><MessageCircle size={20}/><span>Let’s connect on WhatsApp<small>{directWhatsApp ? brand.whatsapp : 'CONTACT DETAILS COMING SOON'}</small></span><ArrowUpRight size={19}/></Button><p className="contact-privacy" data-testid="contact-privacy"><LockKeyhole size={14}/>{directWhatsApp ? 'Continuing opens WhatsApp with your enquiry. Review it there and tap Send to share it with Bhavana.' : 'Your draft stays in this browser unless you choose to copy or download it.'}</p></Reveal>
    <Reveal className="enquiry-form-wrap" delay={0.12}><div className="form-heading"><h3 data-testid="enquiry-title">Make space for yourself.</h3><span data-testid="enquiry-subtitle">Tell us a little about your journey.</span></div><form onSubmit={submit} data-testid="enquiry-form">
      <div className="form-grid">
        ...
        <div className="form-field"><label htmlFor="email">Email address <small>(optional)</small></label><input id="email" name="email" type="email" value={form.email} onChange={update} autoComplete="email" maxLength={254} placeholder="you@example.com" data-testid="enquiry-email"/></div>
      ...
      <Button className="action action-gold form-submit" type="submit" data-testid="enquiry-submit">Continue on WhatsApp<ArrowUpRight size={18}/></Button>
      <p className="form-disclosure" data-testid="enquiry-disclosure">{directWhatsApp ? 'You send the message in WhatsApp. Your preferred date and time are requests; Bhavana will confirm availability and booking details.' : 'WhatsApp enquiries are not available until our contact number is added. For now, this button prepares a draft you can copy or download; nothing is sent.'}</p>
    </form></Reveal>
  </div></section>;
};